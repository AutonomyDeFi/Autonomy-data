...
class Agent(a.Block):
    ...
    def process_example(self, example, natural_lang_input):
            # Split the natural language input into sentences
            sentences = example[natural_lang_input].split('. ')
            
            # Process each sentence as a separate task
            for i, sentence in enumerate(sentences):
                if sentence:  # Check if the sentence is not empty
                    print(f"Processing sentence {i + 1}: {sentence}")
                    task_key = str(i + 1)
                    task = example["tasks"].get(task_key)
                    if task:
                        task_str = task["task"]
                        self.call(task_str, self.memory)
...
if __name__ == "__main__":
    ...
    agent = Agent(tools=["tool.inch.gasprice", "tool.swap", "tool.inch.balances", "tool.lido", "tool.rocket-pool", "tool.get_best_apy", "tool.rocket-pool-stake", "tool.lido-stake", "tool.Inch.get_requested_token_prices", "tool.cheapest_token", "tool.compare_token_price"])

    # Process each example
    for example_key, example_value in EXAMPLES.items():
        print(f"Processing example {example_key}")
        agent.process_example(example_value)
